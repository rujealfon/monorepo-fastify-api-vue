...
| GET | `/api/v1/health/ready` | **Readiness** — `200` when DB is reachable; `503` otherwise |
...
// 503 — unhealthy
{ "success": false, "error": { "code": "SERVICE_UNAVAILABLE", "message": "database unreachable" } }
```
...
- `checkValkey(valkey)` — currently commented out while Valkey is disabled
